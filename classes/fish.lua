...
function _Fish.create(params)
  ...
  -- @DELETE
  local fishScale
  if (fish.sizeGroup == "tiny") then
    fishScale = .6
  elseif (fish.sizeGroup == "small") then
    fishScale = .8
  elseif (fish.sizeGroup == "large") then
    fishScale = 1.2
  elseif (fish.sizeGroup == "tiger") then
    fishScale = 1.2
  else
    fishScale = 1
  end
  ...
  -- Fade the fish in and change its mode to seeking after a short delay
  ...
  function fish:tap(params)
    ...
    fish:moveTo(
      {
        x = params.bobberEdgeX,
        y = params.bobberEdgeY,
        onComplete = function()
  ...
  function fish.los:collision(event)
    ...
    if event.other.myName == "bobber" or event.other.myName == "splash" then
      ...
      if bobber.isActive and fish.mode == "SEEKING" then
        ...
        local delay = 1000
        for i = 1, numTaps do
            tap = timer.performWithDelay(delay, function()
                    fish:tap(
                        {
                            bobberEdgeX = bobberEdge.x,
                            bobberEdgeY = bobberEdge.y,
                            bobberCenterX = x,
                            bobberCenterY = y,
                            lpointX = lookingPoint.x,
                            lpointY = lookingPoint.y
                        }
                    )
                end
            )
            ...
            -- fish is reeled in too quickly
            table.insert(fish.tapTimers, tap)

            delay = delay + 2500
        end

        bite = timer.performWithDelay(delay, function()
          fish:moveTo(
            {
              x = bobberEdge.x,
              y = bobberEdge.y,
              onComplete = fish:bite({
                bobberX = x, 
                bobberY = y
              })
            })
        ...
        table.insert(fish.tapTimers, bite)
      end
    end
  end
