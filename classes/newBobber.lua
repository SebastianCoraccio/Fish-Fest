...
function _Bobber.create(x, y, group)
  ...
  function bobber:bringBack()
    bobber.isActive = false -- bobber isn't active
    bobber.isCatchable = false -- bobber isn't catchable
    bobber.anim:setLinearVelocity(0, 0) -- stop the bobber
    transition.to(
      bobber.anim,
      {
  ...
  function moveBobber()
    counter = counter + 1
    if (bobber.isActive and bobber.isCatchable and counter % 2 == 0) then
